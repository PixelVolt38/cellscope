...
except ImportError:  # compatibility with newer jupyter_server
    from jupyter_server.extension.handler import ExtensionHandlerMixin
    from jupyter_server.base.handlers import APIHandler

    class ExtensionHandler(ExtensionHandlerMixin, APIHandler):  # type: ignore[misc]
        pass
...
class AnalyzeHandler(ExtensionHandler):
    def post(self):
...
class ExportHandler(ExtensionHandler):
    def post(self):
        data = self.get_json_body() or {}
        nb_path = data.get("notebook")
...
class IndexHandler(ExtensionHandler):
    def post(self):
        data = self.get_json_body() or {}
...
def _to_json(capture: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cells": cells, "edges": capture["graph"]["edges"]}
