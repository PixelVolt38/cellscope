...
- **C1: CellScope** — Analyze + list/filter + graph.
...
## Study design
- **Within-subject:** each participant does both C0 and C1.
...
## Tasks
- Consent + rules (1 min)
- Short training (C1 only) (2 min)
- Task set A (5 min)
- Task set B (5 min)
- Feedback period (5 min)
...
For each task, use the answer key below (cell numbers + header labels). Tasks 1 and 3 use `RAVL_sanitized.ipynb`. Tasks 2 use `multi_kernel_demo.ipynb` + `exhaustive_python.ipynb` to demonstrate cross-notebook provenance. Participants do not know which notebooks to find the answers in, and have access to all 3 notebooks 
...
Prompt: “The file `examples/data_outputs/climate_readings.csv` is produced in `multi_kernel_demo.ipynb` and later read in `exhaustive_python.ipynb`. Identify the producer cell and the first consumer cell.”
...
Not part of the C0 vs C1 comparison.
