...
def cmd_build(args):
    ...
    if not args.no_index:
        ...
        if index_result["endpoint"] and index_result["status"] is not None:
            print(
                f"Index delta ({index_result['triples']} triples) posted to "
                f"{index_result['endpoint']} (status {index_result['status']})"
            )
        else:
            print(
                f"Index delta ({index_result['triples']} triples) written to "
                f"{index_result['output']}"
            )
...
def main():
    ...
    pb.add_argument('--index-endpoint', help='SPARQL endpoint URL to POST the index delta')
    pb.add_argument('--index-output', help='Override path to write the SPARQL update payload')
